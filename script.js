...
async function loadMultiplayerQuestion() {
    ...
    if (randomMode === 'trivia') {
        console.log('Loading trivia question using main game system');
        await loadTriviaQuestion(sport);
    } else if (randomMode === 'college') {
        console.log('Loading college question using main game system');
        await loadCollegeQuestion(sport);
    } else if (randomMode === 'jersey') {
        console.log('Loading jersey question using main game system');
...
async function loadTriviaQuestion(sport) {
    try {
        ...
        // Check if dataLoader is available and wait for it to be ready
        console.log('🔍 Checking dataLoader availability...');
        console.log('dataLoader type:', typeof dataLoader);
        console.log('dataLoader object:', dataLoader);
        console.log('dataLoader.loadTriviaQuestions:', dataLoader?.loadTriviaQuestions);
        
        if (typeof dataLoader === 'undefined' || !dataLoader || !dataLoader.loadTriviaQuestions) {
            console.error('dataLoader not available, waiting for initialization...');
            
            // Wait a bit for dataLoader to be initialized
            await new Promise(resolve => setTimeout(resolve, 500));
            
            console.log('🔍 After wait - dataLoader type:', typeof dataLoader);
            console.log('🔍 After wait - dataLoader object:', dataLoader);
            
            if (typeof dataLoader === 'undefined' || !dataLoader || !dataLoader.loadTriviaQuestions) {
                console.error('dataLoader still not available after wait, using fallback');
        ...
        let triviaQuestions = await dataLoader.loadTriviaQuestions(targetSport);
...
async function loadCollegeQuestion(sport) {
    try {
        ...
        if (typeof dataLoader === 'undefined' || !dataLoader || !dataLoader.loadPlayers) {
            console.error('dataLoader not available for college questions, using fallback');
            const fallbackQuestion = {
                type: 'college',
                question: 'Which college did this player attend?',
                options: ['University of Alabama', 'Ohio State University', 'University of Georgia', 'Clemson University'],
        ...
        console.error('Error loading college question:', error);
        displayMultiplayerQuestion(createFallbackTriviaQuestion(sport));
    }
}
...
async function loadJerseyQuestion(sport) {
    try {
        ...
        // Check if dataLoader is available
        if (typeof dataLoader === 'undefined' || !dataLoader || !dataLoader.loadPlayers) {
            console.error('dataLoader not available for jersey questions, using fallback');
            const fallbackQuestion = {
                type: 'jersey',
                question: 'What jersey number does this player wear?',
                options: ['12', '7', '3', '9'],
                correctAnswer: '12',
                category: 'Jersey',
                difficulty: 'medium'
        ...
        console.error('Error loading jersey question:', error);
        displayMultiplayerQuestion(createFallbackTriviaQuestion(sport));
    }
}

function createFallbackTriviaQuestion(sport) {
    ...
    return fallbackQuestions[sport] || fallbackQuestions.nfl;
}
...
function displayMultiplayerQuestion(question) {
    ...
    try {
        ...
        // Display the question text - use the correct multiplayer element ID
        const questionTextElement = document.getElementById('multiplayer-question-text');
        console.log('🔍 Looking for multiplayer-question-text element...');
        console.log('🔍 Found element:', questionTextElement);
        console.log('🔍 Current screen:', currentScreen);
        
        if (questionTextElement) {
            const oldText = questionTextElement.textContent;
            questionTextElement.textContent = question.question || 'No question available';
            console.log('📝 Question text updated from:', oldText, 'to:', question.question);
            console.log('📝 Element after update:', questionTextElement.textContent);
        } else {
            console.error('❌ Multiplayer question text element not found!');
            console.log('🔍 Available elements with "question" in ID:');
    ...
    if (multiplayerGame.gameSettings.inputType === 'multiple') {
        ...
        console.log('🔀 Original choices order:', choices);
        choices = shuffleArray([...choices]); // Create a copy and shuffle it
        console.log('🔀 Shuffled choices order:', choices);
        ...
        choices.forEach((choice, index) => {
            ...
            if (isCorrect) {
                ...
                console.log('✅ Correct answer is at position:', index, 'Text:', choiceText);
            }
            
            button.onclick = () => selectMultiplayerAnswer(choiceText);
...
function selectMultiplayerAnswer(selectedText) {
    const question = multiplayerGame.currentQuestion;
    let isCorrect = false;

    // Check if the selected text matches the correct answer
    if (question.correctAnswer !== undefined) {
        isCorrect = selectedText === question.correctAnswer;
    } else if (question.correctChoiceId !== undefined) {
        // Fallback for old format
        const selectedChoice = question.choices ? question.choices[selectedText] : question.options[selectedText];
        const choiceText = typeof selectedChoice === 'string' ? selectedChoice : selectedChoice.text;
        isCorrect = choiceText === question.correctAnswer;
    }

    console.log('🎯 Answer selected:', selectedText, 'Correct:', isCorrect, 'Expected:', question.correctAnswer);
